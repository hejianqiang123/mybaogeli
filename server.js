...
express.get("/addStudent",(request,response)=>{
	...
	if(name && age && sex && city && day){
		sql.query(`INSERT INTO students (name,sex,age,city,day) VALUES ("${name}","${sex}","${age}","${city}","${day}")`,(error,data)=>{
			if(error){
				console.log(error)
				response.end("error")
			}
...
//修改信息
express.get("/compile",(request,response)=>{
	const id = request.query.id;
	const name = request.query.name;
	const age = request.query.age; 	
	const sex = request.query.sex;
	...
	const day = request.query.day;
	if(name && age && sex && city && day){
		sql.query(`UPDATE students SET name="${name}",age="${age}",city="${city}",day="${day}",sex="${sex}"WHERE id="${id}"`,(error,data)=>{
			if(error){
				console.log(error)
				response.end("error")
			}	
